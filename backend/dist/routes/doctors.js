...
router.get('/:doctorId/info', async (req, res) => {
...
router.get('/stats/all', async (req, res) => {
    try {
        const { data: doctors, error: doctorsError } = await supabase_1.supabase
            .from('profiles')
            .select('*')
            .eq('role', 'doctor')
...
router.post('/second-opinion/request', async (req, res) => {
    const { childId, reportId, requestedDoctorId, notes } = req.body;
    const parentId = req.headers['x-user-id']; // Should come from auth middleware
...
router.get('/available/second-opinion/:childId', async (req, res) => {
    const { childId } = req.params;
    try {
        // Get the child with assigned doctor
        const { data: child, error: childError } = await supabase_1.supabase
            .from('children')
