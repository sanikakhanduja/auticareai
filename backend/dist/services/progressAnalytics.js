...
class TherapyProgressAnalytics {
    ...
    static async computeProgress(childId, therapyType) {
        ...
        try {
            ...
            const previousSessions = await this.fetchPreviousPeriodSessions(childId, therapyType, CONFIG.COMPARISON_PERIOD_DAYS);
            ...
            throw new Error(`Failed to compute progress: ${error.message}`);
        }
    }
    // =====================================================
    // DATA FETCHING
     ...
     */
    static async fetchRecentSessions(childId, therapyType, days) {
     ...
     */
    static async fetchPreviousPeriodSessions(childId, therapyType, days) {
     ...
     */
    static computeAverages(sessions) {
        const metrics = [
            'eye_contact_score',
    ...
    static computeTrends(sessions) {
        ...
        const mostCommon = trendValues.sort((a, b) => trendValues.filter(v => v === a).length - trendValues.filter(v => v === b).length).pop() || 'stable';
     ...
     */
    static computeChanges(previous, recent) {
        const changes = {};
     ...
     */
    static detectRegression(changes) {
     ...
     */
    static computeConsistency(sessions) {
     ...
     */
    static analyzeMetrics(averages) {
     ...
     */
    static async checkStagnation(childId, therapyType) {
