...
exports.llmExplanationService = exports.TherapyProgressExplanationService = void 0;
// =====================================================
// PROMPT TEMPLATES
// =====================================================
class TherapyProgressExplanationService {
    /**
     * Generate role-specific explanation
     * Calls LLM with structured data
     */
    static async generateExplanation(request) {
...
`.trim();
     ...
     */
    static getParentPrompt(data, request) {
        return `
...
`.trim();
     ...
     */
    static getTherapistPrompt(data, request) {
        return `
...
`.trim();
     ...
     */
    static getDoctorPrompt(data, request) {
        return `
...
`.trim();
     ...
     */
    static async callLLM(prompt, role) {
     ...
     */
    static parseResponse(llmResponse, role) {
    ...
    static getConsistencyLevel(score) {
        if (score >= 0.8)
    ...
    static buildFallbackSummary(request) {
    ...
    static async resolveGeminiModel(apiKey) {
        ...
        try {
            ...
            if (listResponse.ok) {
                ...
                const supported = models.filter((m) => Array.isArray(m?.supportedGenerationMethods) &&
                    m.supportedGenerationMethods.includes('generateContent') &&
                    typeof m?.name === 'string');
                const supportedNames = supported.map((m) => String(m.name).replace(/^models\//, ''));
    ...
    static extractRetrySeconds(message) {
...
exports.TherapyProgressExplanationService = TherapyProgressExplanationService;
TherapyProgressExplanationService.resolvedGeminiModel = null;
TherapyProgressExplanationService.llmBlockedUntilMs = 0;
TherapyProgressExplanationService.GEMINI_MODEL_CANDIDATES = [
    'gemini-2.0-flash',
    'gemini-1.5-flash',
    'gemini-1.5-pro',
];
...
// EXPORT
// =====================================================
exports.llmExplanationService = {
    async generateExplanation(request) {
        return await TherapyProgressExplanationService.generateExplanation(request);
    },
    async generateMultiRoleExplanations(analytics, childName, therapyType, milestones) {
